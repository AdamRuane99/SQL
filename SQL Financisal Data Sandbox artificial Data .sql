...
InflowOutflow AS(
 ...
 , 
 InflowOutflowTable AS(
 SELECT 
 [Month End], Month_Name, Month_Number, [YEAR],  [Code], [DESCRIPTION CODE], [Units], [Units Value] 
 ,  [Amount],
  Country,   [Product Code], POLICY_NUMBER,
 
 SUM(CASE WHEN [DESCRIPTION CODE] IN ('Surrender', 'PARTIAL SURRENDER'   , 'MATURITY') THEN Amount END) AS [Out Flows], 
 SUM(CASE WHEN [DESCRIPTION  CODE] IN 'SPI', 'INSTALLMENT', 'FIRST PREMIUM', 'COUPON RE-INVESTMENT') THEN Amount END) AS [In Flows]
 FROM InflowOutflow
